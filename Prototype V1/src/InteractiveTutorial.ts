...
export function mountInteractiveTutorial(tutorialModal: HTMLElement) {
  ...
  overlay.innerHTML = `
    <div class="cit-panel" role="dialog" aria-modal="true" aria-labelledby="cit-title">
      ...
      <section class="cit-view" data-cit-view="classes"><h3>Preview each skill, then try it</h3><p class="cit-status">Use the individual <strong>PREVIEW EFFECT</strong> buttons to see what a skill is intended to do. The Q/E/R buttons on the right simulate activating it and update the demo HUD.</p><div class="cit-class-row" data-cit-class-row></div><div class="cit-demo-grid"><div class="cit-demo-card"><h4 data-cit-class-name>Tank</h4><p data-cit-class-passive></p><p data-cit-class-q></p><p data-cit-class-e></p><p data-cit-class-r></p><div class="cit-actions"><button class="cit-action" data-cit-skill-preview="Q">PREVIEW Q EFFECT</button><button class="cit-action" data-cit-skill-preview="E">PREVIEW E EFFECT</button><button class="cit-action" data-cit-skill-preview="R">PREVIEW R EFFECT</button></div></div><div class="cit-demo-card"><p class="cit-status" data-cit-skill-status>Choose a class, then preview or fire Q, E, or R.</p><div class="cit-actions"><button class="cit-action" data-cit-skill="Q">FIRE Q</button><button class="cit-action" data-cit-skill="E">FIRE E</button><button class="cit-action primary" data-cit-skill="R">FIRE R ULTIMATE</button></div><div class="cit-meter"><i data-cit-skill-meter style="width:65%"></i></div><p class="cit-status">Demo meter: <span data-cit-skill-lines>26</span> / <span data-cit-skill-cost>50</span> lines</p></div></div></section>
    </div>`;
  ...
  overlay.querySelectorAll<HTMLButtonElement>('[data-cit-skill-preview]').forEach(b=>b.addEventListener('click',()=>{const slot=b.dataset.citSkillPreview!; const info=PLAYER_CLASSES.find(x=>x.id===selectedClass)!; const description=slot==='Q'?info.abilityQDescription:slot==='E'?info.abilityEDescription:info.ultimateDescription; const status=overlay.querySelector<HTMLElement>('[data-cit-skill-status]')!; status.textContent=`${info.name} ${slot} preview: ${description}`; const card=b.closest('.cit-demo-card'); card?.animate([{transform:'scale(1)',boxShadow:'0 0 0 rgba(0,229,255,0)'},{transform:'scale(1.02)',boxShadow:'0 0 26px rgba(0,229,255,.35)'},{transform:'scale(1)',boxShadow:'0 0 0 rgba(0,229,255,0)'}],{duration:420});}));
