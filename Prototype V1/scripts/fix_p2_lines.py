...
for i, line in enumerate(lines):
    if "const p2 = opIdx >= 0 ? gameManager.players[opIdx] : undefined;" in line:
        start_idx = i
    if start_idx != -1 and i > start_idx and "Update multiplayer scoreboard" in line:
        end_idx = i
        break
...
"""
    new_lines.append(new_p2_block)
    new_lines.extend(lines[end_idx:])
    
    with open('src/main.ts', 'w', encoding='utf-8') as f:
        f.writelines(new_lines)
    print("Replaced P2 block safely!")
else:
